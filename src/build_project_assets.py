...
employees = {}
def is_recruited(x): return np.random.choice([0, 1], p=[1 - x, x])
...
df = df.merge(notes[['employee_id',
                     'event_date',
                     'note']],
              on=['employee_id',
                  'event_date'],
              how='left').merge(notes[['employee_id',
                                       'employee_name']].drop_duplicates(),
                                on=['employee_id'])
...
y = X.join(df.drop_duplicates('employee_id').set_index(
